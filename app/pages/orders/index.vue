...
const { allOrders, allOrdersLoading, allOrdersPagination, fetchAllOrders } = useOrders()
...
const showPagination = computed(() => {
   return allOrdersPagination.value.lastPage > 1 && allOrdersPagination.value.total > 0
})
...
<template>
   ...
   <div v-if="showPagination" class="mt-6 flex justify-center gap-2">
      <button 
         v-for="page in allOrdersPagination.lastPage" 
         :key="page"
         :class="[
