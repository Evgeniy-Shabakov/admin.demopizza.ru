...
const columns = [
  ...
  { key: 'usedAt', label: 'Когда применил', mobileLabel: 'Когда', render: (item) => item.usedAt 
    ? `<span class="inline-flex px-2 py-1 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 rounded font-medium">${formatDateTime(item.usedAt)}</span>` 
    : '—' },
  { key: 'createdAt', label: 'Создано', mobileLabel: 'Создано', render: (item) => formatDateTime(item.createdAt) },
  { key: 'updatedAt', label: 'Обновлено', mobileLabel: 'Обновлено', render: (item) => formatDateTime(item.updatedAt) },
