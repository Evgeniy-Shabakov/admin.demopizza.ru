    ...
    <template v-else-if="promocode">
      <BaseCard class="p-4 md:py-4">
        ...
        <div class="space-y-2 text-sm">
          ...
          <div class="flex justify-between pt-2">
            ...
            <span class="text-gray-900 dark:text-gray-100 text-xs">{{ formatDateTime(promocode.createdAt) }}</span>
          </div>

          <div class="flex justify-between">
            <span class="text-gray-500 dark:text-gray-400">Обновлен</span>
            <span class="text-gray-900 dark:text-gray-100 text-xs">{{ formatDateTime(promocode.updatedAt) }}</span>
          </div>
        </div>
      </BaseCard>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <BaseCard class="p-4 md:p-3">
          ...
          <p class="text-sm text-gray-500 dark:text-gray-400">{{ formatDateTime(promocode.usedAt) }}</p>
        </BaseCard>
      </div>
    </template>
