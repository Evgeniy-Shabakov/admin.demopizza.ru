...
const canChangePayment = computed(() => {
   return order.value?.paymentTypeId === PAYMENT_TYPE.CASH.ID ||
      order.value?.paymentTypeId === PAYMENT_TYPE.CARD_OFFLINE.ID
})
...
const orderStatusName = computed(() => {
   return ORDER_STATUS_NAME_BY_ID[order.value.orderStatusId]
})

const orderTypeName = computed(() => {
   return ORDER_TYPE_NAME_BY_ID[order.value.orderTypeId]
})

const paymentTypeName = computed(() => {
   return PAYMENT_TYPE_NAME_BY_ID[order.value.paymentTypeId]
})

const paymentStatusName = computed(() => {
   return PAYMENT_STATUS_NAME_BY_ID[order.value.paymentStatusId]
})
...
async function nextStatus() {
   ...
   try {
      ...
      order.value.orderStatusId = response.data.data.orderStatusId
   } catch (e) {
...
async function cancelOrder() {
   ...
   try {
      ...
      order.value.orderStatusId = ORDER_STATUS.CANCEL.ID
...
const updatePaymentStatus = async () => {
   ...
   try {
      const newStatus = order.value.paymentStatusId === PAYMENT_STATUS.NO_PAID.ID
         ? PAYMENT_STATUS.PAID.ID
         : PAYMENT_STATUS.NO_PAID.ID
      await $api.patch(`/orders/${order.value.id}`, { paymentStatusId: newStatus })
      showToast('Статус оплаты изменён', 'success')
      order.value.paymentStatusId = newStatus
...
watch(() => order.value, () => {
   if (order.value) {
      useHead({
         title: `${order.value.number} ${order.value.totalPrice} ₽`
      })
   }
})
</script>
...
<template>
   ...
   <div v-else-if="order"
        class="space-y-4 md:space-y-6 max-w-4xl">
      <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-3 md:p-6">
         <div class="flex flex-wrap items-center justify-between gap-x-4 gap-y-2 mb-3">
            ...
            <span class="text-gray-500 dark:text-gray-400 text-sm md:text-xl md:flex-1 md:text-center"
                  title="время создания заказа">{{ formatOrderTime(order.createdAt) }}</span>
            <div v-if="order.city?.name"
                 class="w-full text-sm text-gray-500 dark:text-gray-400 md:w-auto md:text-right mt-1 md:mt-0">
               <span v-if="order.restaurant?.name"
                     class="font-semibold text-gray-900 dark:text-white">{{ order.restaurant.name }}</span>
         ...
         <div class="text-center mb-4 md:mb-6 pb-4 md:pb-6 border-b border-gray-200 dark:border-gray-700">
            <div
                 class="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-2 md:gap-4 w-full mb-3">
               <span
                     class="inline-flex items-center justify-center px-3 md:px-4 py-2 rounded-lg text-sm md:text-base font-semibold bg-teal-100 text-teal-800 dark:bg-teal-900/30 dark:text-teal-400">
                  {{ orderTypeName }}
               </span>
               <span v-if="order.packTakeaway"
                     class="inline-flex items-center justify-center gap-1 md:gap-2 px-3 md:px-4 py-2 rounded-lg text-sm md:text-base font-semibold bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-400">
                  <svg class="w-4 md:w-5 h-4 md:h-5"
                       fill="none"
                       stroke="currentColor"
                       viewBox="0 0 24 24">
               ...
               <span v-else
                     class="inline-flex items-center justify-center gap-1 md:gap-2 px-3 md:px-4 py-2 rounded-lg text-sm md:text-base font-semibold bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400">
                  <svg class="w-4 md:w-5 h-4 md:h-5"
                       viewBox="0 0 24 24"
                       fill="currentColor">
                     <path
            ...
            <div v-if="order.orderStatusId"
                 :class="['px-4 py-2 rounded-lg text-base font-semibold text-center block w-full',
                  getStatusClass(order.orderStatusId)]">
               {{ orderStatusName }}
            </div>
         </div>

         <div v-if="!order.userId"
              class="text-center mb-2 rounded p-4
         ...
         <div class="flex flex-col md:flex-row items-start justify-between gap-4 md:gap-6 mb-4 md:mb-6">
            <div class="flex-1 w-full">
               <div class="text-sm md:text-base text-blue-600 dark:text-blue-400 flex items-center gap-2 mb-2">
                  <svg class="w-4 md:w-5 h-4 md:h-5 text-blue-500 dark:text-blue-400"
                       fill="none"
                       stroke="currentColor"
                       viewBox="0 0 24 24">
                     <path stroke-linecap="round"
               ...
               <div class="text-sm md:text-base text-green-600 dark:text-green-400 flex items-start gap-2">
                  <svg class="w-4 md:w-5 h-4 md:h-5 text-green-500 dark:text-green-400 shrink-0 mt-0.5"
                       fill="none"
                       stroke="currentColor"
                       viewBox="0 0 24 24">
                     <path stroke-linecap="round"
               ...
               <div v-if="order.deliveryZone?.name"
                    class="text-sm md:text-base text-gray-500 dark:text-gray-400 mt-1 ml-6 md:ml-7">
                  Зона доставки: {{ order.deliveryZone.name }}
               </div>
               <div v-else-if="order.address"
                    class="text-sm md:text-base text-green-600 dark:text-green-400 flex items-start gap-2">
                  <svg class="w-4 md:w-5 h-4 md:h-5 text-green-500 dark:text-green-400 shrink-0 mt-0.5"
                       fill="none"
                       stroke="currentColor"
                       viewBox="0 0 24 24">
            ...
            <div class="flex flex-col items-start md:items-end gap-1.5 md:gap-2 w-full md:w-auto">
               ...
               <span v-if="order.paymentTypeId"
                     ...
                     class="text-xs md:text-sm flex items-center gap-1">
                  <svg class="w-3.5 md:w-4 h-3.5 md:h-4 shrink-0"
                       fill="none"
                       stroke="currentColor"
                       viewBox="0 0 24 24">
                     <path stroke-linecap="round"
               ...
               <span v-if="order.banknoteForChange"
                     class="text-xs md:text-sm bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 rounded px-2 py-1">Сдача
         ...
         <div v-if="order.tableNumber || order.carNumber || order.leaveAtTheDoor || order.dontRingDoorbell || order.userComment"
              class="border-t border-gray-200 dark:border-gray-700 pt-4 md:pt-6 mb-4 md:mb-6 flex flex-wrap gap-2 md:gap-3">
            <div v-if="order.tableNumber"
                 class="text-sm md:text-base bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 rounded-lg px-2.5 md:px-3 py-1">
            ...
            <div v-if="order.userComment"
                 class="text-sm md:text-base bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 rounded-lg px-2.5 md:px-3 py-1"
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4 md:p-6">
         <div class="text-sm md:text-base text-gray-500 dark:text-gray-400 mb-3 md:mb-4">
            Позиций: <span class="text-gray-900 dark:text-white font-medium">{{ positionsCount }}</span> | Товаров:
            <span class="text-gray-900 dark:text-white font-medium">{{ totalItemsCount }}</span>
         </div>

         <div class="space-y-3 md:space-y-4">
            <div v-for="item in order.orderProducts"
                 :key="item.productId"
                 class="flex items-center gap-2 md:gap-4 p-2 md:p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
               <img v-if="getProductImageUrl(item.product?.imagePath)"
                     :src="getProductImageUrl(item.product?.imagePath)"
                    :alt="item.product.name"
                    class="w-10 h-10 md:w-14 md:h-14 rounded-lg object-cover">
               <div v-else
                    class="w-10 h-10 md:w-14 md:h-14 rounded-lg bg-gray-200 dark:bg-gray-600 flex items-center justify-center">
                  <svg class="w-4 md:w-6 h-4 md:h-6 text-gray-400"
                       fill="none"
                       stroke="currentColor"
                       viewBox="0 0 24 24">
                     <path stroke-linecap="round"
               ...
               <div class="flex-1 min-w-0">
                  <span class="text-sm md:text-lg font-normal md:font-medium text-gray-900 dark:text-white">{{
                     item.product?.name }}</span>
               </div>
               <div class="text-right shrink-0">
                  <span class="text-sm md:text-xl font-normal md:font-semibold text-gray-900 dark:text-white">{{
                     item.quantity }} шт.</span>
                  <div class="text-xs md:text-sm text-gray-500 dark:text-gray-400">({{ item.quantity }} × {{ item.price
         ...
         <div v-if="order.totalProductsPrice !== undefined || order.totalPrice !== undefined"
              class="border-t border-gray-200 dark:border-gray-700 mt-4 md:pt-6 pt-4 space-y-2">
            ...
            <div v-if="order.totalPrice !== undefined"
                 class="flex justify-between text-lg md:text-xl font-semibold pt-2">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4 md:p-6">
         <div :class="['px-3 md:px-4 py-2 rounded-lg text-sm md:text-base font-semibold text-center block w-full mb-3 md:mb-4',
            getStatusClass(order.orderStatusId)]">
            {{ orderStatusName }}
         </div>
