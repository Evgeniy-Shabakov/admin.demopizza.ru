...
const getProductImage = (item) => {
  if (item.product?.imagePath) {
    return getImageUrl(item.product.imagePath)
  }
  return null
}
...
const columns = [
  { key: 'id', label: 'ID' },
  { key: 'product', label: 'Товар', white: true, render: (item) => {
  ...
  { key: 'restaurantName', label: 'Ресторан', white: true, render: (item) => item.restaurant?.name || item.restaurantId },
  { key: 'isInStopList', label: 'В стоп-листе', render: (item) => item.isInStopList ? '<span class="text-red-600">Да</span>' : '<span class="text-green-600">Нет</span>' },
   { key: 'createdAt', label: 'Создано', showInMobile: false, render: (item) => formatDateTime(item.createdAt) },
   { key: 'updatedAt', label: 'Обновлено', showInMobile: false, render: (item) => formatDateTime(item.updatedAt) },
