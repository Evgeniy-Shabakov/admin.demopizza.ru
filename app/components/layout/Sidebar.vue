...
const menuGroups = computed(() => {
   ...
   props.menuItems.forEach(item => {
      if (item.group) {
         ...
         if (!group) {
            group = { name: item.group, items: [], isExpanded: ref(true), showOnClick: ref(false) }
            groups.push(group)
         }
...
const toggleGroup = (group) => {
   if (props.isCollapsed) {
      group.showOnClick.value = !group.showOnClick.value
   } else {
...
<div :class="isCollapsed ? 'lg:hidden' : ''">
       ...
       <div class="flex-1 overflow-y-auto">
          ...
          <template v-for="item in menuGroups.standalone" :key="item.path">
...
<svg v-if="item.icon === 'home'"
            ...
            <svg v-else-if="item.icon === 'briefcase'"
                 class="w-5 h-5"
                 fill="none"
                 stroke="currentColor"
                 viewBox="0 0 24 24">
            ...
            <svg v-else-if="item.icon === 'document'"
                 class="w-5 h-5"
                 fill="none"
                 stroke="currentColor"
                 viewBox="0 0 24 24">
            ...
            <svg v-else-if="item.icon === 'ticket'"
                 class="w-5 h-5"
                 fill="none"
                 stroke="currentColor"
                 viewBox="0 0 24 24">
            ...
            <svg v-else-if="item.icon === 'shopping-cart'"
                 class="w-5 h-5"
                 fill="none"
                 stroke="currentColor"
                 viewBox="0 0 24 24">
            ...
            <svg v-else-if="item.icon === 'clipboard-list'"
                 class="w-5 h-5"
                 fill="none"
                 stroke="currentColor"
                 viewBox="0 0 24 24">
                ...
                <svg v-else-if="item.icon === 'palette'"
                 class="w-5 h-5"
                 fill="none"
                 stroke="currentColor"
                 viewBox="0 0 24 24">
          ...
          <div v-for="group in menuGroups.groups" :key="group.name" class="space-y-1 lg:relative group">
            <button @click="toggleGroup(group)"
                    :class="[
                      'flex items-center gap-2 w-full px-4 py-2 text-sm font-medium rounded-lg transition-colors cursor-pointer',
                      isCollapsed ? 'lg:w-full lg:justify-center lg:px-2' : '',
                      isGroupActive(group)
                         ? 'bg-indigo-50 dark:bg-indigo-900/50 text-indigo-600 dark:text-indigo-400'
                         : 'text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700'
                    ]">
              <svg class="w-4 h-4 transition-transform"
                   :class="[(!isCollapsed && group.isExpanded.value) || (isCollapsed && group.showOnClick.value) ? 'rotate-0' : '-rotate-90']"
                   fill="none"
                   stroke="currentColor"
                   viewBox="0 0 24 24">
                <path stroke-linecap="round"
              ...
              <span :class="isCollapsed ? 'lg:hidden' : ''">{{ group.name }}</span>
            </button>
            <div v-show="(!isCollapsed && group.isExpanded.value) || (isCollapsed && group.showOnClick.value)"
                  :class="[
                      isCollapsed 
                        ? 'lg:flex lg:flex-col lg:gap-1 lg:mt-1 lg:pb-2' 
                        : ''
                    ]">
              <NuxtLink v-for="item in group.items"
                        :key="item.path"
                        :to="item.path"
                        @click="handleMenuClick"
                        :class="[
               ...
               <svg v-else-if="item.icon === 'palette'"
                ...
                <span v-show="!isCollapsed" class="">{{ item.name }}</span>
              </NuxtLink>
             </div>
           </div>
         </nav>
